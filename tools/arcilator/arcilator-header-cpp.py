...
extern ArcRuntimeModelInfo model_info_{{ model.name }};
#endif
}
...
public:
  ...
  explicit {{ model.name }}(const char* runtimeArgs = nullptr)  :
    storage(arcRuntimeAllocateInstance(&model_info_{{ model.name }}, runtimeArgs)->modelState),
    view(&storage[0])
  {
{% if model.initialFnSym %}
...
{% endif %}
  ...
  ~{{ model.name }}() {
    ArcState *instance = arcRuntimeGetStateFromModelState(storage, 0);
    arcRuntimeDeleteInstance(instance);
  }
