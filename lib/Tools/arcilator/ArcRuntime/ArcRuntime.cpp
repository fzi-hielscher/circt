...
ArcState *arcRuntimeAllocateInstance(const ArcRuntimeModelInfo *model,
  ...
  statePtr->impl =
      new circt::arc::runtime::ModelInstance(model, args, statePtr);
...
void arcRuntimeDeleteInstance(ArcState *instance) {
  if (instance->impl)
    delete reinterpret_cast<circt::arc::runtime::ModelInstance *>(
        instance->impl);
  free(instance);
}
...
const char *APICallbacks::symName_allocInstance = "arcRuntimeIR_allocInstance";
const char *APICallbacks::symName_deleteInstance =
