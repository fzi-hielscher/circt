...
// RUN: circt-opt --test-apply-lowering-options='lowering-options=' --split-input-file --export-verilog %s | FileCheck %s --check-prefix=CLEAR
// RUN: circt-opt --test-apply-lowering-options='lowering-options=noAlwaysComb' --split-input-file --export-verilog %s | FileCheck %s --check-prefix=NOALWAYSCOMB
